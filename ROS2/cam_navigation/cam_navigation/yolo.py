...
# Globals
# Try to find the model in the current directory or the script's directory
_MODEL_FILENAME = "best.pt"
_cwd_path = os.path.abspath(_MODEL_FILENAME)
_script_dir_path = os.path.join(os.path.dirname(__file__), _MODEL_FILENAME)

if os.path.exists(_cwd_path):
    MODEL_PATH = _cwd_path
elif os.path.exists(_script_dir_path):
    MODEL_PATH = _script_dir_path
else:
    raise FileNotFoundError(f"Could not find '{_MODEL_FILENAME}' in current directory or script directory.")
...
if __name__ == "__main__":
    ...
    if not cap.isOpened():
        print("Could not open webcam (index 0). Trying Jetson GStreamer pipeline...")
        cap = cv2.VideoCapture(GST_PIPELINE, cv2.CAP_GSTREAMER)
        if not cap.isOpened():
            raise RuntimeError("Could not open webcam (index 0) or Jetson camera pipeline.")
