...
class LocalSignDetector(LocalSignDetector):
    def __init__(self,
        model_path,
        cls_thres=None,
        signs=None,
    ...
    def _load_classifier(self, model_path):
        ...
        if len(self.class_names) != num_output:
            raise ValueError(f"Mismatch: {len(self.class_names)} etiquetas vs. {num_output} salidas del modelo.")
    ...
    def _segment_by_color(self, hsv):
        masks = {}
    ...
    def _extract_rois(self, frame):
        hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
        masks = self._segment_by_color(hsv)
        rois = []
    ...
    def non_max_suppression(self, boxes, scores, classes):
        ...
        indices = cv2.dnn.NMSBoxes(boxes, scores, score_threshold=min_thresh, nms_threshold=self.IOU_THRESHOLD)
        if not len(indices):
            return []
        idxs = indices.flatten()
        return [(boxes[i][0], boxes[i][1], boxes[i][2], boxes[i][3], classes[i], scores[i]) for i in idxs]
    
    def detect_classes(self, frame, drawing_frame=None):
